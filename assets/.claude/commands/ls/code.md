...
> **本命令的所有构建/测试命令都从 `ls-pipeline.config.md` 读取**（位置见 AGENTS.md/CLAUDE.md 指引，通常在项目根），而不是硬编码某个工具。开工前先读该配置，取出这些字段：`shell`、`build`（编译/类型检查）、`unit-test`（跑单测：整体命令 + 单测试文件/类模板）、`coverage`（覆盖率工具+阈值，或 `none`）、`module-layout`（如何指定单个模块/包）。下文用 `<build>`/`<unit-test>`/`<single-test>`/`<coverage>` 代指配置里的对应命令。若 config 缺失，停下提示用户先按模板创建。
...
4. **每组 task 后：全量构建**
   执行 config 的 `<build>`（编译/类型检查整个工程）。构建成功才继续下一组。
